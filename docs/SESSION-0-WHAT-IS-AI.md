...
> Let me ask you something. [Show a few examples of messy handwritten 3s on screen.] What number is this? Right — it's a 3. You knew that instantly. But now try to write a program to do that. A traditional computer program. You'd need rules like:
>
> "If there are two curves stacked on top of each other, open to the left..." But what counts as a curve? How do you define "stacked"? What about someone who writes their 3s with sharp angles instead of curves? What about someone whose 3 looks like a backwards E? Every rule you write has a hundred edge cases, and the edge cases have their own edge cases.
>
> This is the fundamental problem that neural networks were invented to solve. There's a whole category of tasks — recognizing images, understanding speech, reading handwriting — that are trivial for the human brain but essentially impossible to code with traditional if/then rules. You can't write enough rules. There are too many exceptions.
>
> So the insight was: instead of *telling* the computer how to recognize a 3, what if we *showed* it thousands of 3s and let it figure it out on its own? That's the core idea behind machine learning — and neural networks are the architecture that makes it possible.
>
> We're going to use handwritten digit recognition as our running example for the first half of this session. It's the "hello world" of AI — simple enough to understand, but it contains every concept that scales up to ChatGPT.
...
> OK, first building block. When you hear the word "neuron" in the context of AI, forget everything you know about biology. An AI neuron is not a brain cell. It is simply a container that holds a number — specifically, a number between 0 and 1.
>
> Think of it like a dimmer switch on a light. All the way down is 0 — off. All the way up is 1 — fully on. And it can be anywhere in between. 0.73, 0.12, 0.95 — any value in that range. We call this value the neuron's "activation." When the number is close to 1, we say the neuron is "lit up" or "activated." When it's close to 0, the neuron is quiet.
>
> That's it. That's a neuron. A box with a number in it. All of the incredible things AI can do — every conversation with ChatGPT, every image generated by Midjourney, every recommendation Netflix gives you — comes from networks of these simple number-holding boxes interacting with each other. The magic is not in any single neuron. The magic is in how they're connected.
...
> Now we stack neurons into layers, and this is where the architecture starts to emerge. For our digit recognition example, here's the setup:
>
> The images we're working with are 28 pixels wide by 28 pixels tall. That's 784 pixels total. Each pixel has a brightness value — 0 for black, 1 for white, with grays in between. So the input layer has 784 neurons, one per pixel. Each neuron simply holds the brightness of its pixel. Nothing fancy — it's just the raw data coming in.
>
> Then we have two hidden layers with 16 neurons each. Why 16? Honestly, it's somewhat arbitrary — you could experiment with 32, or 64, or different numbers of layers entirely. The specific numbers are less important than the concept.
>
> And the output layer has 10 neurons — one for each digit, 0 through 9. When you feed in an image, whichever output neuron lights up the brightest is the network's answer. If the "3" neuron has an activation of 0.95 and everything else is below 0.1, the network is confidently saying "that's a 3."
>
> The key idea here is **hierarchy**. The first hidden layer might learn to detect simple things — edges, small line segments, corners. The second hidden layer takes those edges and combines them into higher-level patterns — loops (like in a 0, 6, 8, or 9), vertical lines (like in a 1), horizontal crossbars (like in a 7). And the output layer takes those patterns and maps them to specific digits.
>
> This hierarchical approach isn't unique to image recognition. Speech recognition works the same way — raw audio gets broken into distinct sounds, then syllables, then words, then phrases. Each layer handles a different level of abstraction. It's like your military chain of command — the front line collects raw intelligence, the middle echelons analyze and synthesize, and the top makes the final call. Each level does one job, and the information flows up.
...
> So we have layers of neurons. How does information flow from one layer to the next? Through connections — and each connection has a weight.
>
> A weight is just a number assigned to a connection between two neurons. Think of it like a volume knob on each wire. Positive weight means "if the neuron feeding into me lights up, I should light up too." Negative weight means the opposite — "if that neuron lights up, I should stay *quiet*." And the size of the weight controls how strong the effect is.
>
> Here's a concrete example. Imagine a neuron in the first hidden layer whose job is to detect a specific edge — say, a short diagonal line in the upper left of the image. You'd want positive weights (shown in blue) on the pixels where that edge would appear, and negative weights (shown in red) on the surrounding pixels. Why negative? Because you don't just want to detect bright pixels — you want to detect bright pixels in *that specific pattern*, surrounded by darkness. The negative weights help distinguish a deliberate edge from a random bright blob.
>
> If you arrange those weights into a 28×28 grid matching the image dimensions, you can literally *see* what pattern each neuron is looking for. The blue region is where it wants to see brightness; the red region is where it wants darkness.
>
> Now here's the key insight: every neuron in one layer connects to *every* neuron in the next layer. For our network, that's 784 input neurons connecting to 16 hidden neurons — that's 784 × 16 = 12,544 connections just in the first layer, each with its own weight. Plus 16 × 16 between the hidden layers, plus 16 × 10 to the output. Add in the biases — which we'll cover next — and you get 13,002 total adjustable numbers. Those 13,002 numbers *are* the network. They define everything it knows and everything it can do.
...
> Now let's look at what a single neuron actually *computes*. Every neuron in the hidden and output layers does the exact same four-step process:
>
> **Step 1:** Take every neuron feeding into me, and multiply its activation by the weight on that connection. So if a neuron in the previous layer has an activation of 0.9 and the weight on the connection is 0.8, that contributes 0.9 × 0.8 = 0.72 to my sum.
>
> **Step 2:** Add up all of those products. If I have 784 inputs, that's 784 multiplications added together into one big number.
>
> **Step 3:** Add a bias. The bias is a separate number — think of it as a threshold or a minimum bar. A bias of -10 means the weighted sum has to be greater than 10 before this neuron starts to care. Grant Sanderson describes it as "how big that weighted sum needs to be before the neuron gets meaningfully active." It's like a hiring manager who weighs qualifications — the weights — but still requires a minimum score before they'll even schedule an interview. That minimum score is the bias.
>
> **Step 4:** Squish the result. The weighted sum plus bias could be any number — 50, -200, 3.7 — but neuron activations need to be between 0 and 1. So we run the result through what's called an activation function. The original one was called the **sigmoid function** — it takes any number and smoothly squishes it into the 0-to-1 range. Very negative numbers get squished toward 0, very positive numbers get squished toward 1, and numbers near zero end up around 0.5.
>
> Modern networks actually use something simpler called **ReLU** — Rectified Linear Unit. It's dead simple: if the number is negative, output 0. If it's positive, pass it through unchanged. The reason the field switched from sigmoid to ReLU is practical: sigmoid gets extremely flat at the extremes, which means when you're trying to train the network, tiny adjustments to weights produce almost no change in the output. The network gets stuck. ReLU never flattens, so training stays responsive.
>
> But the concept is the same: multiply inputs by weights, add them up, add a bias, and squeeze the result into a valid range. That's it. That is the one operation that every neuron performs. This one simple operation, repeated thousands of times across layers, is what produces all the complex behavior you see in AI.
...
> I want to pause here and make sure this point lands, because it's the most important demystification I can offer.
>
> A neural network is a function. Full stop. It's a mathematical function. You put 784 numbers in one end — the pixel values — math happens in the middle — weighted sums, biases, activation functions — and 10 numbers come out the other end — the probabilities for each digit. If you wanted, you could write the whole thing out as one enormous mathematical expression. In fact, it can be written very compactly using matrix notation: the activation of each layer is just σ(W·a + b), where W is the weight matrix, a is the input vector, b is the bias vector, and σ is the squishing function.
>
> The function has 13,002 adjustable parameters — those are the weights and biases. With *random* parameters, the function is complete garbage. It outputs random nonsense. With the *right* parameters — parameters that were learned from thousands of labeled examples — it recognizes handwriting with 98% accuracy.
>
> So the structure — the layers, the connections, the activation functions — is designed by humans. We set the architecture. But the specific parameter values — the 13,002 numbers that make it work — are *learned from data*. And that learning process is what we're going to cover next.
>
> As Grant Sanderson puts it: once you understand this, "it feels a lot less like some crazy sci-fi premise and a lot more like a calculus exercise." It's just finding the minimum of a function. So let's talk about how that works.
