...
**Playlist Position:** 06

---

## Transcript

[00:00] The initials GPT stand for Generative Pretrained Transformer.
[00:05] So that first word is straightforward enough, these are bots that generate new text.
[00:09] Pretrained refers to how the model went through a process of learning
[00:13] from a massive amount of data, and the prefix insinuates that there's
[00:16] more room to fine-tune it on specific tasks with additional training.
[00:20] But the last word, that's the real key piece.
[00:23] A transformer is a specific kind of neural network, a machine learning model,
[00:27] and it's the core invention underlying the current boom in AI.
[00:31] What I want to do with this video and the following chapters is go through a
[00:35] visually-driven explanation for what actually happens inside a transformer.
[00:39] We're going to follow the data that flows through it and go step by step.
[00:43] There are many different kinds of models that you can build using transformers.
[00:47] Some models take in audio and produce a transcript.
[00:51] This sentence comes from a model going the other way around,
[00:54] producing synthetic speech just from text.
[00:56] All those tools that took the world by storm in 2022 like DALL-E and Midjourney
[01:01] that take in a text description and produce an image are based on transformers.
[01:06] Even if I can't quite get it to understand what a pi creature is supposed to be,
[01:09] I'm still blown away that this kind of thing is even remotely possible.
[01:13] And the original transformer introduced in 2017 by Google was invented for
[01:18] the specific use case of translating text from one language into another.
[01:22] But the variant that you and I will focus on, which is the type that
[01:26] underlies tools like ChatGPT, will be a model that's trained to take in a piece of text,
[01:31] maybe even with some surrounding images or sound accompanying it,
[01:34] and produce a prediction for what comes next in the passage.
[01:38] That prediction takes the form of a probability distribution
[01:41] over many different chunks of text that might follow.
[01:45] At first glance, you might think that predicting the next word
[01:47] feels like a very different goal from generating new text.
[01:50] But once you have a prediction model like this,
[01:52] a simple thing you could try to make it generate, a longer piece of text,
[01:56] is to give it an initial snippet to work with,
[01:58] have it take a random sample from the distribution it just generated,
[02:02] append that sample to the text, and then run the whole process again to make
[02:05] a new prediction based on all the new text, including what it just added.
[02:10] I don't know about you, but it really doesn't feel like this should actually work.
[02:13] In this animation, for example, I'm running GPT-2 on my laptop and having it repeatedly
[02:17] predict and sample the next chunk of text to generate a story based on the seed text.
[02:22] The story just doesn't actually really make that much sense.
[02:26] But if I swap it out for API calls to GPT-3 instead, which is the same basic model,
[02:31] just much bigger, suddenly almost magically we do get a sensible story,
[02:35] one that even seems to infer that a pi creature would live in a land of math and
[02:40] computation.
[02:41] This process here of repeated prediction and sampling is essentially
[02:44] what's happening when you interact with ChatGPT,
[02:47] or any of these other large language models, and you see them producing
[02:50] one word at a time.
[02:52] In fact, one feature that I would very much enjoy is the ability to
[02:55] see the underlying distribution for each new word that it chooses.
[03:03] Let's kick things off with a very high level preview
[03:06] of how data flows through a transformer.
[03:08] We will spend much more time motivating and interpreting and expanding
[03:11] on the details of each step, but in broad strokes,
[03:14] when one of these chatbots generates a given word, here's what's going on under the hood.
[03:19] First, the input is broken up into a bunch of little pieces.
[03:22] These pieces are called tokens, and in the case of text these tend to be
[03:26] words or little pieces of words or other common character combinations.
[03:30] If images or sound are involved, then tokens could be little
[03:34] patches of that image or little chunks of that sound.
[03:37] Each one of these tokens is then associated with a vector, meaning some list of numbers,
[03:42] which is meant to somehow encode the meaning of that piece.
[03:45] If you think of these vectors as giving coordinates in some very high dimensional space,
[03:50] words with similar meanings tend to land on vectors that are
[03:53] close to each other in that space.
[03:55] This sequence of vectors then passes through an operation that's
[03:58] known as an attention block, and this allows the vectors to talk to
[04:01] each other and pass information back and forth to update their values.
[04:04] For example, the meaning of the word model in the phrase "a machine learning
[04:08] model" is different from its meaning in the phrase "a fashion model".
[04:12] The attention block is what's responsible for figuring out which
[04:15] words in context are relevant to updating the meanings of which other words,
[04:19] and how exactly those meanings should be updated.
[04:22] And again, whenever I use the word meaning, this is
[04:25] somehow entirely encoded in the entries of those vectors.
[04:29] After that, these vectors pass through a different kind of operation,
[04:32] and depending on the source that you're reading this will be referred
[04:35] to as a multi-layer perceptron or maybe a feed-forward layer.
[04:38] And here the vectors don't talk to each other,
[04:40] they all go through the same operation in parallel.
[04:43] And while this block is a little bit harder to interpret,
[04:45] later on we'll talk about how the step is a little bit like asking a long list
[04:49] of questions about each vector, and then updating them based on the answers
[04:53] to those questions.
[04:54] All of the operations in both of these blocks look like a
[04:58] giant pile of matrix multiplications, and our primary job is
[05:01] going to be to understand how to read the underlying matrices.
[05:06] I'm glossing over some details about some normalization steps that happen in between,
[05:10] but this is after all a high-level preview.
[05:13] After that, the process essentially repeats, you go back and forth
[05:17] between attention blocks and multi-layer perceptron blocks,
[05:20] until at the very end the hope is that all of the essential meaning
[05:24] of the passage has somehow been baked into the very last vector in the sequence.
[05:28] We then perform a certain operation on that last vector that produces a probability
[05:33] distribution over all possible tokens, all possible little chunks of text that might
[05:37] come next.
[05:38] And like I said, once you have a tool that predicts what comes next
[05:42] given a snippet of text, you can feed it a little bit of seed text and
[05:45] have it repeatedly play this game of predicting what comes next,
[05:49] sampling from the distribution, appending it, and then repeating over and over.
[05:53] Some of you in the know may remember how long before ChatGPT came into the scene,
[05:57] this is what early demos of GPT-3 looked like,
[06:00] you would have it autocomplete stories and essays based on an initial snippet.
[06:05] To make a tool like this into a chatbot, the easiest starting point is to have a
[06:09] little bit of text that establishes the setting of a user interacting with a
[06:13] helpful AI assistant, what you would call the system prompt,
[06:17] and then you would use the user's initial question or prompt as the first bit of
[06:21] dialogue, and then you have it start predicting what such a helpful AI assistant
[06:25] would say in response.
[06:27] There is more to say about an added step of training that's required
[06:30] to make this work well, but at a high level this is the idea.
[06:35] In this chapter, you and I are going to expand on the details of what happens at the very
[06:39] beginning of the network, at the very end of the network,
[06:42] and I also want to spend a lot of time reviewing some important bits of background
[06:46] knowledge, things that would have been second nature to any machine learning engineer by
[06:50] the time transformers came around.
[06:53] If you're comfortable with that background knowledge and a little impatient,
[06:56] you could probably feel free to skip to the next chapter,
[06:58] which is going to focus on the attention blocks,
[07:00] generally considered the heart of the transformer.
[07:03] After that, I want to talk more about these multi-layer perceptron blocks,
[07:06] how training works, and a number of other details that will have been skipped up to
[07:11] that point.
[07:12] For broader context, these videos are additions to a mini-series about deep learning,
[07:16] and it's okay if you haven't watched the previous ones,
[07:18] I think you can do it out of order, but before diving into transformers specifically,
[07:22] I do think it's worth making sure that we're on the same page about the basic premise
[07:27] and structure of deep learning.
[07:29] At the risk of stating the obvious, this is one approach to machine learning,
[07:33] which describes any model where you're using data to somehow determine how a model
[07:37] behaves.
[07:39] What I mean by that is, let's say you want a function that takes in
[07:42] an image and it produces a label describing it,
[07:44] or our example of predicting the next word given a passage of text,
[07:48] or any other task that seems to require some element of intuition
[07:51] and pattern recognition.
[07:53] We almost take this for granted these days, but the idea with machine learning is that
[07:57] rather than trying to explicitly define a procedure for how to do that task in code,
[08:02] which is what people would have done in the earliest days of AI,
[08:05] instead you set up a very flexible structure with tunable parameters,
[08:09] like a bunch of knobs and dials, and then, somehow,
[08:11] you use many examples of what the output should look like for a given input to tweak
[08:16] and tune the values of those parameters to mimic this behavior.
[08:19] For example, maybe the simplest form of machine learning is linear regression,
[08:24] where your inputs and outputs are each single numbers,
[08:27] something like the square footage of a house and its price,
[08:30] and what you want is to find a line of best fit through this data, you know,
[08:34] to predict future house prices.
[08:37] That line is described by two continuous parameters,
[08:40] say the slope and the y-intercept, and the goal of linear
[08:43] regression is to determine those parameters to closely match the data.
[08:48] Needless to say, deep learning models get much more complicated.
[08:52] GPT-3, for example, has not two, but 175 billion parameters.
[08:58] But here's the thing, it's not a given that you can create some giant
[09:01] model with a huge number of parameters without it either grossly
[09:05] overfitting the training data or being completely intractable to train.
[09:10] Deep learning describes a class of models that in the
[09:13] last couple decades have proven to scale remarkably well.
[09:16] What unifies them is that they all use the same training algorithm,
[09:19] it's called backpropagation, we talked about it in previous chapters,
[09:22] and the context that I want you to have as we go in is that in order for this
[09:26] training algorithm to work well at scale, these models have to follow a certain
[09:30] specific format.
[09:31] And if you know this format going in, it helps to explain many of the choices for how a
[09:36] transformer processes language, which otherwise run the risk of feeling kinda arbitrary.
[09:41] First, whatever kind of model you're making, the
[09:43] input has to be formatted as an array of real numbers.
[09:46] This could simply mean a list of numbers, it could be a two-dimensional array,
[09:50] or very often you deal with higher dimensional arrays,
[09:53] where the general term used is tensor.
[09:56] You often think of that input data as being progressively transformed into many
